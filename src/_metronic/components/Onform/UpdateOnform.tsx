...
export default function UpdateOnform(props: any) {
    ...
    console.log('color@@@EData', teamData)
...
const handleStatusColor = () => {
        ...
        if (teamData.pv === true) {
            ...
            if ((teamData?.nichtGeeignet === false || teamData?.nichtGeeignet === null) && teamData?.emailFailed === null) {
                if (!teamData?.pv && !teamData?.sms && !teamData?.contactedBy && !teamData?.contactedOn && (teamData?.contactedAgain === "Invalid date" && teamData?.contactedAgain===null && teamData?.contactedAgain.length===0) 
                &&( teamData?.lastContact === "Invalid date" && teamData?.lastContact === null && teamData?.lastContact?.length === 0) && !teamData?.reached && !teamData?.makeAppointment
                    && !teamData?.usefulInformation && (teamData?.appointmentDate === "Invalid date"  && teamData?.appointmentDate === null && teamData?.appointmentDate?.length === 0) && !teamData?.appointmentTime) {
                    console.log("first")
                    return color = "red"
                } else {
                    if ((teamData?.appointmentDate !== "Invalid date" && teamData?.appointmentDate !== null && teamData?.appointmentDate.length!==0 ) || teamData.appointmentTime) {
                        console.log("second")
                        return color = "green"
                    } else {
                        console.log("third")
                ...
                if (teamData?.nichtGeeignet === true || teamData?.emailFailed === true || teamData?.emailFailed === null) {
                    console.log("fifth")
                    return color = "black"
                } else {
                    if ((teamData?.appointmentDate !== "Invalid date" && teamData?.appointmentDate?.length !== 0  && teamData?.appointmentDate !== null  ) || teamData?.appointmentTime   ) {
                        console.log("sixth")
                        return color = "green"
                    } else {
                        if (teamData?.sms || teamData?.contactedBy || (teamData?.contactedOn !== "Invalid date" &&  teamData?.contactedOn?.length !== 0 && teamData?.contactedOn !== null)
                            ||( teamData?.contactedAgain !== "Invalid date" && teamData?.contactedAgain?.length !== 0 && teamData?.contactedAgain !== null ) ||(teamData?.lastContact !== "Invalid date" && teamData.lastContact?.length !== 0 && teamData.lastContact !== null )
                            || teamData?.reached || teamData?.makeAppointment || teamData?.usefulInformation) {
                            console.log("seventh")
                            return color = "orange"
                        } else {
                            console.log("eigth")
